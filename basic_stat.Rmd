...
svm_se = sqrt(sum((pre_result_train - train_data$revenue)^2))
svm_se
...
var_se  <- data.frame(var_name = all_var)
for (variable in all_var){
  obj <- tune.svm(revenue~., data = train_data[, !(colnames(train_data) %in% c("Id", variable))], gama = 2^(-10:1), cost = 2^(2:10))
  model <- obj$best.model
  pre_result_train <- predict(model, subset(train_data, select = Open.Date:P37))
  var_se[var_se$var_name == variable, "se"] = sqrt(sum((pre_result_train - train_data$revenue)^2))
}
```
...
```{r}
library(ggplot2)
var_se$se = var_se$se - svm_se
ggplot(var_se, aes(x = var_name, y = se)) + geom_bar(stat = "identity")
```
...
pre_train_svm <- predict(model_list[[bestcut]], subset(train_data,select = Open.Date:P37))
...
dim(model$importance[model$importanceSD > 1.5e10, ])

selected <- rownames(model$importance[model$importanceSD > 1.5e10, ])

model_rf <- randomForest(revenue~.,data=subset(train_data, select = names(train_data) %in% c("revenue", selected)))
pre_train_forest <- predict(model_rf, subset(train_data,select = Open.Date:P37))
sqrt(sum((pre_train_forest - train_data$revenue)^2))
...
sqrt(sum((pre_train_combine - train_data$revenue)^2))

pre_test_svm <- predict(model_svm, subset(test_data, select = -Id))
pre_test_forest <- predict(model_forest, subset(test_data, select = -Id))
